...
parser.add_argument("--config", type=str, default="configs/qwen2.5-omni-3b/morpho/scienceqa_morpho.yaml", help="Path to the yaml configuration file")
...
# ========== 设置搜索超参数 ==========
if QUANT_METHOD in ("morpho", "morpho_withhif8", "morpho_withhif4"):
    LP_NORM = getattr(config.quant, "lp_norm", 0.5)
    USE_COSINE_LOSS = getattr(config.quant, "use_cosine_loss", False)
    print(f"🚀 [搜索策略调整] 当前量化搜索评价指标使用的 lp_norm 为: {LP_NORM}")
    print(USE_COSINE_LOSS)
...
if __name__ == "__main__":
    ...
    print(f"开始在 ScienceQA 测试集上评测模型，结果保存到 {out_log} ...")
    acc = evaluator.evaluate(save_dir=out_log)
